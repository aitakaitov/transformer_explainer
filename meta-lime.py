...
from transformer_explainer.explainers import ExplainerLIME
...
explanation = ExplainerLIME(model, tf.gather(input_embeds, 0)).explain(input_embeds=np.array(input_embeds),
                                                attention_mask=np.array(encoded.data['attention_mask']),
                                                token_type_ids=np.array(encoded.data['token_type_ids']),
                                                tokens=tokens, sample_count=500)
