...
from transformer_explainer.explainers import ExplainerSmoothGRAD
...
explanation = ExplainerSmoothGRAD(model).explain(input_embeds=np.array(input_embeds),
                                                attention_mask=np.array(encoded.data['attention_mask']),
                                                token_type_ids=np.array(encoded.data['token_type_ids']),
                                                tokens=tokens, steps=100)
